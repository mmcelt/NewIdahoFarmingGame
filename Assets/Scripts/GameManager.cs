...
public class GameManager : MonoBehaviourPun
{
	...
	void Start() 
	{
		...
		if (PhotonNetwork.IsConnectedAndReady)
		{
			...
			if (PhotonNetwork.LocalPlayer.CustomProperties.TryGetValue(IFG.Selected_Farmer, out selectedFarmer))
			{
				...
				Vector3 spawnPosition = _playerSpawnPoints[farmerIndex].position;

				InstantiateFarmer(farmerIndex, spawnPosition);

				StartCoroutine(GameLoopRoutine());//INACTIVE FOR INITAL TESTING
			}
		}
	}
	#endregion
	...
	int GetIndexFromName(string farmer)
	{
		...
		if (farmer == IFG.Ron)
			return 0;
		else if (farmer == IFG.Kay)
			return 1;
		else if (farmer == IFG.Jerry)
			return 2;
		else if (farmer == IFG.Ric)
			return 3;
		else if (farmer == IFG.Becky)
			return 4;
		else if (farmer == IFG.Mike)
			return 5;
		else
		{
	...
	Color GetPlayerColorFromIndex(int index)
	{
	...
	{
		GameObject myPlayer = PhotonNetwork.Instantiate(_playerPrefab.name, spawnPosition, Quaternion.identity);

		myPlayer.GetComponentInChildren<SpriteRenderer>().color = GetPlayerColorFromIndex(farmerIndex);
	}
